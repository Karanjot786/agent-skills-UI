...
| `--all` | Install to **all 45 agents** at once |
| `-g, --global` | Install globally (user-wide, home directory) |
| `-s, --skill <skills...>` | Install by marketplace skill name(s) — supports glob patterns |
...
| `--dry-run` | Preview matched skills without installing |
...
# All skills to all 45 agents, fully non-interactive
...
#### Glob/Wildcard Matching (`--skill` + patterns)

Use `*` and `?` in skill names to match patterns. Works with both marketplace search and git-cloned repos.

```bash
# Marketplace glob: install all python-related skills
skills install -s 'python-*' -a claude

# Preview without installing
skills install -s 'python-*' -a claude --dry-run

# Glob within a cloned repo
skills install owner/repo --skill 'core-*' -a cursor
skills install owner/repo --skill 'test-?' --dry-run

# Multiple patterns
skills install -s 'react-*' 'vue-*' -a claude
```

**Output (dry-run):**
```
✔ Found 60 skill(s) matching "python-*"
  @Shubhamsaboo/python-expert
  @wshobson/python-packaging
  @wshobson/python-performance-optimization
  ...
```

#### Branch Targeting (`#branch` suffix)

Append `#branch` to SSH or shorthand URLs to target a specific branch.

```bash
# Shorthand with branch
skills install owner/repo#dev -a claude
skills install owner/repo#v2 -a cursor -y

# SSH with branch
skills install git@github.com:owner/repo.git#feature -a claude

# HTTP uses /tree/branch instead (existing behavior)
skills install https://github.com/owner/repo/tree/dev -a cursor
```
