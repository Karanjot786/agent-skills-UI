...
order: 16
icon: "layers"
description: "Architecture deep-dive: how the CLI supports 45 agents through adapters"
commands: []
---

The CLI uses a formal adapter pattern to support 45 agents through a single codebase. Each agent gets its own adapter that handles path generation, config rendering, and format detection.
